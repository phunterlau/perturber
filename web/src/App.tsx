...
function QuickProbe({ setStatus, onPromoted }: { setStatus: (value: string) => void; onPromoted: () => void }) {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      setSummary(loaded); setRunId(completed); setStatus(`Completed immutable run ${completed}`); setTab("overview"); setRuns(await listRuns());
  ...
  const promote = async () => {
    if (!runId) return;
    try { await createCase({ ...DEFAULT_INTENT, hypothesis: "Investigate the promoted Quick Probe perturbation." }, workflowFromQuick(spec), runId); setStatus("Promoted immutable rank run into a research case."); onPromoted(); }
    catch (caught) { setError(caught instanceof Error ? caught.message : String(caught)); }
  };
  const openRun = async (id: string) => { const loaded = await loadSummary(id); if (isRankSummary(loaded)) { setSummary(loaded); setRunId(id); setTab("overview"); } };
...
function FFNDecomposition({ rank, residual }: { rank: RankSummary; residual: number }) {
  const top = [...rank.layers].sort((a, b) => b.rms_mass - a.rms_mass)[0];
  return <section className="panel decomposition"><h3>FFN decomposition</h3><div className="decomp-flow"><div><span>Measured ΔF</span><strong>{fmt(rank.measured_delta_mean)}</strong></div><i>−</i><div><span>Predicted ΣI</span><strong>{fmt(rank.predicted_delta_mean)}</strong></div><i>=</i><div className={Math.abs(residual) > Math.abs(rank.measured_delta_mean) * .5 ? "alert" : ""}><span>Residual / skip</span><strong>{fmt(residual)}</strong></div><i>·</i><div><span>Leading layer</span><strong>{top ? `L${top.layer}` : "—"}</strong><small>{top ? `${(top.top_10_rms_share * 100).toFixed(1)}% in top 10` : ""}</small></div></div></section>;
}
...
function FFNLayers({ summary }: { summary: RankSummary | null }) {
  if (!summary) return <Empty>Layer ranking is available on rank runs.</Empty>; const sorted = [...summary.layers].sort((a, b) => b.rms_mass - a.rms_mass);
  return <section className="panel chart"><h3>Signed contribution by layer</h3><Plot data={[{ type: "bar", x: summary.layers.map((x) => x.layer), y: summary.layers.map((x) => x.signed_mean_sum), marker: { color: summary.layers.map((x) => x.signed_mean_sum >= 0 ? "#0e9b8d" : "#c94b78") }, hovertemplate: "Layer %{x}<br>ΣI %{y:.5f}<extra></extra>" }]} layout={{ ...plotLayout, xaxis: { title: "Layer", gridcolor: "#dce5e8" }, yaxis: { title: "Signed ΣI", gridcolor: "#dce5e8" } }} config={{ displaylogo: false, responsive: true }} useResizeHandler style={{ width: "100%", height: 340 }} /><div className="layer-strip">{sorted.slice(0, 8).map((item) => <span key={item.layer}>L{item.layer}<strong>{item.rms_mass.toFixed(3)}</strong></span>)}</div></section>;
}
